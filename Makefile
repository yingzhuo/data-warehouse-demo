...
usage:
	...
	@echo "clean         -> 清理构建产物"
	@echo "github        -> 推送代码到https://github.com/yingzhuo/data-warehouse-demo"

build-jar:
	@mvn -f $(CURDIR)/pom.xml clean package -D version=$(version)

build-image: build-jar
	@docker image build --tag 192.168.99.115/data-warehouse/business-sub-system:latest $(CURDIR)/01-business-sub-system/target/docker-context/
